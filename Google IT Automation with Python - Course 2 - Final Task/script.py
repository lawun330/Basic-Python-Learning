...
'''This script creates CSV files, for errors and users, based on system log errors and infos.'''
...
with open('syslog.log') as file: # change the file name here
    for line in file.readlines():
        user = re.search(r"ticky: (INFO|ERROR):* ([\w ']*)(\[[#\d]*\])*.*\(([a-zA-Z\. ]+)\)",line) # regex for the log
        if user:
